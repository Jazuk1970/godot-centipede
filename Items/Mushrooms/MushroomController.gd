...
#Preload scenes
var oMushroom = preload("res://Items/Mushrooms/Mushroom.tscn")
...
func _in_play() -> void:
	...
	Globals.no_of_mushrooms_in_player_zone  = _count_playerzone(Globals.player_zone)
...
func convertMushroom(_id:int):
	var _mob = mushrooms[_id]
	_mob.mushroom_type = Globals.mushroom_types.POISON
	_mob.setFrame(_mob.curFrame)

func removeMushroom(_id:int):
	var _mob = mushrooms[_id]
	Globals.grid_set_cell(_mob.grid_pos,Globals.grid_types.MUSHROOM,Globals.grid_types.EMPTY)
	_mob.queue_free()
	mushrooms.erase(_id)
...
func _count_playerzone(_area:Rect2) -> int:
	var _count:int = 0
	var _grid = Globals._get_grid(Globals.grid_types.MUSHROOM)
	for _x in _area.end.x:
		for _y in range(_area.position.y,_area.end.y):
			if _grid[_x][_y] != Globals.grid_types.EMPTY:
				_count += 1
	return _count
