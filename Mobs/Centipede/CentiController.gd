...
var _times_to_kill:int = 0
var _times_killed:int = 0
...
func _level_up() -> void:
	_times_killed += 1
	if  _times_killed >= _times_to_kill:
		_times_killed = 0
		Globals.Level +=1
...
func _reset_centipede() -> void:
	...
	if Globals.Level == 1 or Globals.Score  >= 40000:
		_times_to_kill = 1
	else:
		_times_to_kill = 2
