...
#Preload scenes
var oMOB = preload("res://Mobs/Spider/Spider.tscn")
...
var _min_time:float = 2.0
var _max_time:float = 12.0
var _chance:int = 2 # 1 in 2 change for example
var state:int
...
func _spawn() -> void:
	...
	var _spawn_y_pos = Globals.getRand_Range(Globals.player_zone.position.y - 6,Globals.player_zone.end.y - 4,0)
	...
	_new_mob._target_direction = _spawn_dir
	_new_mob.add_to_group(mobName)
	_new_mob.add_to_group("enemy")
	_new_mob.connect("remove_mushroom",Globals.mushroom_controller,"removeMushroom")
